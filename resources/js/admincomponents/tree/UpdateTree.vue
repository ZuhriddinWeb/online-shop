               ...
               <main class="flex flex-col h-full justify-between w-full items-center ">
                   <div class="flex justify-between w-full bg-gray-100 px-4 py-2">
                       ...
                       <header class="flex justify-center text-gray-400 items-center text-xl">                       
                           Tip ma'lumotlarini tahrirlash:
                       </header>
                   </div>
                   <form class="w-full  mb-8 mt-8 p-4" @submit.prevent="onSubmit">                   
                   <p>Tip nomi:</p>
                   <input v-model="result.name" class="mb-2 appearance-none bg-transparent w-full text-gray-700 mr-3 px-2 leading-tight border-b border-gray-300 py-2 focus:outline-none" type="text" />
...
const onSubmit = async () => {    
    const { data } = await axios.post(`tree-update/${store.state.id_selected}`, result);
    if (data.status == 200) {
        emit("added");
        emit("close");
    }
